			...
			<tbody>
				...
				<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
					...
					<tr>
						<td scope="row"><?= $no;  ?></td>
						<td>
							<img src="image/produk/<?= $row['gambar']; ?>" width="100">
						</td>
						<td>
							<?= $row['nama']; ?>
						</td>
						<td>
							Rp. <?= number_format($row['hrg']); ?>
						</td>
						<td>
							<input type="number" name="qty" class="form-control" style="text-align: center; width: 30%"
								value="<?= $row['jml']; ?>">
						</td>
						<td>
						...
						<td>
							...
							<a href="keranjang.php?del=1&id=<?= $row['keranjang']; ?>" class="btn btn-danger btn-sm"
				...
				<tr>
					<td colspan="8" style="text-align: right; font-weight: bold;"><a href="index.php"
							class="btn btn-success">Lanjutkan Belanja</a> <a
							href="checkout.php?kode_cs=<?= $kode_cs; ?>" class="btn btn-black">Checkout</a></td>
				</tr>
			</tbody>

			<?php
				} else {
					echo "<h2 class='text-center'>Maaf, belanjaan anda masih kosong</h2>";
				}
			} else {
				echo "<h2 class='text-center'>Anda harus login terlebih dahulu</h2>";
			}
			?>
		</table>
	</div>
